  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-500 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gradient-to-br from-sky-100 via-blue-50 to-indigo-100 text-gray-800'}`}>
      ...
      <header className={`sticky top-0 z-40 w-full border-b ${darkMode ? 'border-gray-700 bg-gray-900/80 backdrop-blur-md' : 'border-gray-200 bg-white/80 backdrop-blur-md'}`}>
        <div className="container mx-auto flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
          <Link to="/" className="flex items-center space-x-2">
            <img  alt="SportConnect Logo" class="h-8 w-auto" src="https://upload.wikimedia.org/wikipedia/commons/d/db/Looo_UADE.svg" />
            <span className={`font-bold text-2xl ${darkMode ? 'text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-blue-500' : 'text-transparent bg-clip-text bg-gradient-to-r from-sky-600 to-blue-700'}`}>
              Zona Deport 
            </span>
          </Link>
          ...
          <nav className="hidden md:flex items-center space-x-2 lg:space-x-4">
            {navLinks.map(link => (
              <Button key={link.to} variant="ghost" asChild className={`${darkMode ? 'hover:bg-gray-700' : 'hover:bg-sky-100'}`}>
                <Link to={link.to} className="flex items-center text-sm font-medium">
      ...
      <main className="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Outlet context={{ currentUser, darkMode, setCurrentUser }} />
      </main>
